...
from gbicom.items import GbicomItem
...
class GBIcomSpider(scrapy.Spider):
    ...
    def crawling_data(self, response):
        print(response.url)
        item = GbicomItem()
        item['brand_name'] = (response.xpath('//*[@id="brandName"]/text()').extract_first()).splitlines()[0]
        item['category'] = response.xpath(
            '//*[@id="content_header"]/div/div[2]/div/div[1]/table/tbody/tr[1]/td[1]/text()').extract_first()
        item['expiration_date'] = response.xpath(
            '//*[@id="content_header"]/div/div[2]/div/div[1]/table/tbody/tr[1]/td[2]/text()').extract_first()
        item['similar_group'] = response.xpath(
            '//*[@id="content_header"]/div/div[2]/div/div[1]/table/tbody/tr[3]/td/a/text()').extract()
        item['projectname'] = response.xpath(
            '//*[@id="content_header"]/div/div[2]/div/div[1]/table/tbody/tr[3]/td/a/text()').extract()
        item['projectname'] = response.xpath(
            '//*[@id="content_header"]/div/div[2]/div/div[1]/table/tbody/tr[4]/td/a/em/text()').extract_first()
        item['ID'] = response.xpath('//*[@id="content_header"]/div/div[1]/div[2]/div[1]/span/em/text()').extract_first()
        item['img_url'] = response.xpath('//*[@id="content_header"]/div/div[1]/div[1]/div/img/@src').extract_first()
